...
public abstract class AbstractStatisticsGatheringHttpClient implements HttpClient {
	private HashMap<Integer,ConnectionStatistics> connectionStats = new HashMap<Integer,ConnectionStatistics>();
     ...
     */
    protected AbstractStatisticsGatheringHttpClient(
	        			...
	        			for(Integer key : connectionStats.keySet()) {
	        				...
	        				if(stats.getConnectionState().equals(ConnectionStatistics.State.OPEN)) {
	        					if(System.currentTimeMillis() - stats.getLastUsed() > 30000 ) {
	        						log.warn("Possible connection leak for connection "+ key + " duration(ms): " +  (System.currentTimeMillis() - stats.getLastUsed()) + " request: " + stats.getLastRequest());
	        					}
	        				}
	        			}
					}
    ...
    protected RequestDirector createClientRequestDirector(
            ...
            final UserTokenHandler stateHandler,
            final HttpParams params) {
        return new StatiscicsLoggingRequestDirector(
                log,
                requestExec,
                conman,
                reustrat,
	...
	public HashMap<Integer,ConnectionStatistics> getConnectionStats() {
